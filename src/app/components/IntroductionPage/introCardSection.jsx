...
const IntroCardSection = () => {
  ...
  return (
    <div className='mt-16 px-10'>
      <h3 className='text-5xl'><b>What we offer the table</b></h3>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} className='mt-4'>
        {cards?.map((card, index) => (
          <Grid item xs={12} sm={6} md={3} key={index}>
            <div className="bg-[#800080] flex gap-2 h-28 rounded-lg mt-2 py-2 px-4">
              <span className='text-white text-lg'>{card?.icon}</span>
              <Typography variant="p" component="div" className='text-white'>
                {card?.mainTxt}
              </Typography>
            </div>
          </Grid>
